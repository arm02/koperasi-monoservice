...
class Lapb_koperasi_pinjaman_konsumtif extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Pinjaman Konsumtif';
		...
		$config = array();
		$config["base_url"] = base_url() . "lapb_koperasi_pinjaman_konsumtif/index/halaman";
		...
		$this->data['isi'] = $this->load->view('laporan/laporan_koperasi/pinjaman_konsumtif', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->lap_simpanan_m->lap_koperasi_pinjaman_konsumtif($offset,$limit,$search);
		$i	= 0;
		$rows   = array();
		if($data){
	...
	function cetak() {
		...
		$data   = $this->lap_simpanan_m->lap_koperasi_pinjaman_konsumtif(0,0,$search);
		$i	= 0;
		$result   = array();
		if($data){
			foreach ($data["rows"] as $r) {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Rekapitulasi Pinjaman Konsumtif Tahun Buku </span>', $width = '100%', $spacing = '1', $padding = '1', $border = '0', $align = 'center').'';
		...
		// $html .= '</table>';
		$pdf->nsi_html($html);
		$pdf->Output('lap_koperasi_pinjaman_konsumtif'.date('Ymd_His') . '.pdf', 'I');
	}
}