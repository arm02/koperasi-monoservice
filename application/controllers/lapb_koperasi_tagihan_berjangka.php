...
class Lapb_koperasi_tagihan_berjangka extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Tagihan Berjangka';
		...
		$this->data['isi'] = $this->load->view('laporan/laporan_koperasi/tagihan_berjangka', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->lap_simpanan_m->lap_koperasi_tagihan_berjangka($offset,$limit,$tahun);
		$i	= 0;
		$rows   = array();
		if($data){
	...
	function cetak() {
		...
		$data   = $this->lap_simpanan_m->lap_koperasi_tagihan_berjangka(null,null,$tahun);
		$i	= 0;
		$result   = array();
		if($data){
			foreach ($data["rows"] as $r) {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Rekapitulasi Tagihan Berjangka Tahun Buku '. $tahun .' </span>', $width = '100%', $spacing = '1', $padding = '1', $border = '0', $align = 'center').'';
		...
		$pdf->nsi_html($html);
		$pdf->Output('lap_koperasi_tagihan_berjangka_'.date('Ymd_His') . '.pdf', 'I');
	}
}