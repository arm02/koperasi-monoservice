...
class Lapb_koperasi_rekap_jasa_barang extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Rekap Jasa Barang';
		...
		$this->data['isi'] = $this->load->view('laporan/laporan_koperasi/rekap_jasa_barang', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->m_lap_koperasi_piutang->lap_koperasi_jasa_barang($offset,$limit,$search);
		$i	= 0;
		$rows   = array();
		if($data){
	...
	function cetak() {
		...
		$data   = $this->m_lap_koperasi_piutang->lap_koperasi_jasa_barang(0,0,$search);
		$i	= 0;
		$result   = array();
		if($data){
			foreach ($data["rows"] as $r) {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Rekapitulasi Rekap Jasa Barang Tahun Buku </span>', $width = '100%', $spacing = '1', $padding = '1', $border = '0', $align = 'center').'';
