...
class Lapb_koperasi_tagihan_barang extends OperatorController {
	...
	function ajax_list() {
		...
		$data   = $this->lap_simpanan_m->lap_koperasi_tagihan_barang($offset,$limit,$tahun);
		$i	= 0;
		$rows   = array();
		if($data){
	...
	function cetak() {
		...
		$data   = $this->lap_simpanan_m->lap_koperasi_tagihan_barang(null,null,$tahun);
		$i	= 0;
		$result   = array();
		if($data){
			foreach ($data["rows"] as $r) {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Rekapitulasi Tagihan Barang Tahun Buku '. $tahun .' </span>', $width = '100%', $spacing = '1', $padding = '1', $border = '0', $align = 'center').'';
		...
		$pdf->nsi_html($html);
		$pdf->Output('lap_koperasi_tagihan_barang_'.date('Ymd_His') . '.pdf', 'I');
	}
}