...
class Lapb_koperasi_rekap_jasa_konsumtif extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Rekap Jasa Konsumtif';
		...
		$this->data['isi'] = $this->load->view('laporan/laporan_koperasi/rekap_jasa_konsumtif', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->m_lap_koperasi_piutang->lap_koperasi_jasa_konsumtif($offset,$limit,$search);
		$i	= 0;
		$rows   = array();
		if($data){
	...
	function cetak() {
		...
		$data   = $this->m_lap_koperasi_piutang->lap_koperasi_jasa_konsumtif(0,0,$search);
		$i	= 0;
		$result   = array();
		if($data){
			foreach ($data["rows"] as $r) {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Rekapitulasi Rekap Jasa Konsumtif Tahun Buku </span>', $width = '100%', $spacing = '1', $padding = '1', $border = '0', $align = 'center').'';
		...
		$pdf->nsi_html($html);
		$pdf->Output('lapb_rekap_jasa_konsumtif'.date('Ymd_His') . '.pdf', 'I');
	}
}