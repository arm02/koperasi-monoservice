...
class Lapb_koperasi_pinjaman_barang extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Pinjaman Barang';
		...
		$config = array();
		$config["base_url"] = base_url() . "lapb_koperasi_pinjaman_barang/index/halaman";
		...
		// print_r($this->lap_simpanan_m->lap_koperasi_pinjaman_barang(1,100));
		$this->data['isi'] = $this->load->view('laporan/laporan_koperasi/pinjaman_barang', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->lap_simpanan_m->lap_koperasi_pinjaman_barang($offset,$limit,$search);
		$i	= 0;
		$rows   = array();
		if($data){
	...
	function cetak() {
		...
		$data   = $this->lap_simpanan_m->lap_koperasi_pinjaman_barang(0,0,$search);
		$i	= 0;
		$result   = array();
		if($data){
			foreach ($data["rows"] as $r) {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Rekapitulasi Pinjaman Barang Tahun Buku </span>', $width = '100%', $spacing = '1', $padding = '1', $border = '0', $align = 'center').'';
