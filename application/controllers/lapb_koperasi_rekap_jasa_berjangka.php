...
class Lapb_koperasi_rekap_jasa_berjangka extends OperatorController {
	...
	function ajax_list() {
		...
		$data   = $this->m_lap_koperasi_piutang->lap_koperasi_jasa_berjangka($offset,$limit,$search);
		$i	= 0;
		$rows   = array();
		if($data){
	...
	function cetak() {
		...
		$data   = $this->m_lap_koperasi_piutang->lap_koperasi_jasa_berjangka(0,0,$search);
		$i	= 0;
		$result   = array();
		if($data){
			foreach ($data["rows"] as $r) {
